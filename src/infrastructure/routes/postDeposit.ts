...
export function postDeposit(router: Router): void {
    ...
    const controller = new DepositPostController(service)
    controller.run = controller.run.bind(controller)

    router.post('/api/accounts/deposit/:id', new AuthMiddleware().check, controller.run)
}