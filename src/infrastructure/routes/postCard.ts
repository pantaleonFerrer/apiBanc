...
import { CardPostController } from "../controllers/CardPostController";

export function postCard(router: Router): void {
    ...
    const controller = new CardPostController(service)
    controller.run = controller.run.bind(controller)

    router.post('/api/cards', new AuthMiddleware().check, controller.run)
}