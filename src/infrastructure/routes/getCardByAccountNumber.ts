...
import { CardGetByAccountNumber } from "../controllers/CardGetByAccountNumber";

export function getCardByAccountNumber(router: Router): void {
    ...
    const controller = new CardGetByAccountNumber(service)
    controller.run = controller.run.bind(controller)

    router.get('/api/cards/account/:id', new AuthMiddleware().check, controller.run)
}