...
import { CardsGetController } from "../controllers/CardsGetController";

export function getCards(router: Router): void {
    ...
    const controller = new CardsGetController(service)
    controller.run = controller.run.bind(controller)

    router.get('/api/cards', new AuthMiddleware().check, controller.run)
}